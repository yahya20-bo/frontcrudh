...
export class AjoutEntreeTissuComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.bonMouvementService.create('entree-tissu', payload).subscribe({
      next: (res) => {
  ...
  exportToExcel(): void {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Entrée Tissu');
    XLSX.writeFile(wb, 'rapport_entree_tissu.xlsx');
  }
  ...
  exportToPDF(): void {
    ...
    doc.save('rapport_entree_tissu.pdf');
  }
}
